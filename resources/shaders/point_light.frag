...
vec2 isPointInRadius(vec2 pos, vec2 point, float dist) {
	float inside = 1.0;
	float dither = 0.0;
	float currdist = u_radius * dist * dist;
	float dither_threshold = currdist * 0.4;
	vec2 diff = vec2(pos - point);
	if (length(diff) > (currdist + dither_threshold)) {
		inside = 0.0;
	} else if (length(diff) > currdist) {
		float t = mod(point.x, 2.0 * u_px);
		float u = mod(point.y, 2.0 * u_px);
		if ((t == 0.0 && u == 1.0 * u_px) || (t == 1.0 * u_px && u == 0.0)) { dither = 1.0; }
	}
...
void main() {
	...
	for (int i = len; i >= 0; i--) {
		float fi = float(i);
		vec2 pos = u_lights[j];
		vec2 result = isPointInRadius(pos, pixelPoint, fi);
		if (result.x == 1.0) {
